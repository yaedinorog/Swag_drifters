...
export interface Checkpoint extends TrackRect {
  id: string;
}
...
export const track01: TrackConfig = {
  id: "track_01",
...
export function isOnTrack(x: number, y: number, track: TrackConfig): boolean {
  const insideOuter =
    x >= track.outer.x &&
...
export function isInsideRect(x: number, y: number, rect: TrackRect): boolean {
