...
import type { TrackAssetV1, TrackGeometry, Quad } from "./types.js";

// ── Helpers (exact copy of client geometry for isOnTrack parity) ──────────
...
function getClosedCenterline(points: Vector2[]): Vector2[] {
  if (points.length < 3) throw new Error("Track centerline must contain at least 3 points.");
  const clean = [...points];
  if (almostEqual(clean[0], clean[clean.length - 1])) clean.pop();
  return clean;
}
...
function sampleClosedSpline(points: Vector2[], tension = 1): Vector2[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < subdivisions; j++) {
      sampled.push(catmullRomCentripetal(p0, p1, p2, p3, j / subdivisions, tension));
    }
  }
  return sampled;
}
...
function getAveragedNormal(points: Vector2[], index: number): Vector2 {
  ...
  if (Math.abs(avg.x) < 1e-6 && Math.abs(avg.y) < 1e-6) return n2;
  return avg;
}
...
function offsetJoinPoint(points: Vector2[], index: number, halfWidth: number, side: 1 | -1): Vector2 {
  ...
  if (miterLength > halfWidth * 2.2) return fallback;
  ...
  if (sideCheck < 0) return fallback;
...
export function buildTrackGeometry(asset: TrackAssetV1): TrackGeometry {
  ...
  return { sampledCenterline: centerline, quads, leftEdge, rightEdge, bounds: { minX, minY, maxX, maxY } };
}
...
export function isOnTrackFromGeometry(x: number, y: number, geometry: TrackGeometry): boolean {
  if (x < geometry.bounds.minX || x > geometry.bounds.maxX ||
      y < geometry.bounds.minY || y > geometry.bounds.maxY) {
    return false;
  }
  for (const quad of geometry.quads) {
    if (pointInPolygon({ x, y }, quad)) return true;
  }
  return false;
}
...
function onSegment(a: Vector2, b: Vector2, c: Vector2): boolean {
  return (
    Math.min(a.x, c.x) - 1e-6 <= b.x && b.x <= Math.max(a.x, c.x) + 1e-6 &&
